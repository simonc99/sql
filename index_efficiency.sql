...
select 
        /*+ 
               ...
               no_monitoring 
               no_expand 
               index_ffs(ucfield,ucfield_i1) 
               noparallel_index(t,ucfield_i1) 
        */ 
        sys_op_lbid( &m_ind_id ,'L',ucfield.rowid) as block_id,
        count(*)                              as rows_per_block
from 
        ucfield 
--      ucfield sample block (100)
-- where 
--         v1 is not null
-- or      small_pad is not null
group by
        sys_op_lbid( &m_ind_id ,'L',ucfield.rowid)
)
